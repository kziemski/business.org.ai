...
flowchart LR
    ...
    subgraph Process["Create master production schedule"]
        P1["Gather Requirements"]
        P2["Analyze Current State"]
        P3["Create master production schedule"]
        P4["Validate & Review"]
...
sequenceDiagram
    ...
    Warehouse->>Logistics: Initiate create master production schedule request
...
| Create master production schedule | R | A | C | I |
...
|-----|-------------|------|
| Cycle Time | Average time to complete create master production schedule process | Hours/Days |
