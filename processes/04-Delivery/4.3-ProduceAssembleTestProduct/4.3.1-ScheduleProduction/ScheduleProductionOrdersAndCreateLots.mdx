---
id: ScheduleProductionOrdersAndCreateLots
name: "Schedule production orders and create lots"
code: "10308"
hierarchyId: "4.3.1.4"
type: Activity
level: 4
status: complete
parent: "4.3.1"
---

# Schedule production orders and create lots

> Creating a schedule to commence production of orders received, and creating lots to consolidate the processing.

## Overview

Activity 4.3.1.4 is an activity within the Deliver Physical Products framework. 

Creating a schedule to commence production of orders received, and creating lots to consolidate the processing. Plan when the production orders are to be initiated by commencing the operations for processing products/services. Specify which materials to produce, where to produce them, which operations will facilitate this, and on which date production is to start. Define the size of production lots, demarcating the durations of batch production.

## Process Hierarchy

```mermaid
graph TD
    C4["4.0 Deliver Physical Products"]
    C4 --> N4_3["4.3 Produce/Assemble/Test product"]
    N4_3 --> N4_3_1["4.3.1 Schedule production"]
    N4_3_1 --> N4_3_1_4["4.3.1.4 Schedule production orders and create..."]
    style N4_3_1_4 fill:#e1f5fe
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10308 |
| Hierarchy ID | 4.3.1.4 |
| Level | Activity |
| Parent | [4.3.1](../) |
| Sub-Processes | 0 |


## GraphDL Semantic Structure

```graphdl
schedule.ProductionOrdersAndCreateLots
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `schedule` | Primary action |
| Object | `production orders and create lots` | Direct object |



## Process Flow

```mermaid
flowchart LR
    subgraph Inputs["Inputs"]
        I1["Requirements & Standards"]
        I2["Organizational Data"]
        I3["Stakeholder Input"]
    end

    subgraph Process["Schedule production orders and create lots"]
        P1["Gather Requirements"]
        P2["Analyze Current State"]
        P3["Schedule production orders and create lots"]
        P4["Validate & Review"]
        P5["Document & Communicate"]
    end

    subgraph Outputs["Outputs"]
        O1["Updated Deliverables"]
        O2["Performance Records"]
        O3["Stakeholder Communication"]
    end

    Inputs --> Process --> Outputs
    P1 --> P2 --> P3 --> P4 --> P5
```


## Process Sequence

```mermaid
sequenceDiagram
    participant Warehouse as Warehouse
    participant Logistics as Logistics
    participant Carrier as Carrier
    participant Customer as Customer

    Warehouse->>Logistics: Initiate schedule production orders and create lots request
    Logistics->>Logistics: Analyze requirements
    Logistics->>Carrier: Request validation
    Carrier->>Logistics: Provide feedback
    Logistics->>Customer: Update status
    Customer->>Warehouse: Confirm completion
```
## RACI Matrix

| Activity | Production Manager | Supply Chain Director | Quality Assurance Team | Finance Department |
|----------|:-:|:-:|:-:|:-:|
| Gather Requirements | R | A | C | I |
| Analyze Current State | R | I | C | I |
| Schedule production orders and create lots | R | A | C | I |
| Validate & Review | C | A | R | I |
| Document & Communicate | R | I | I | C |

## Related Occupations

- [Supply Chain Manager](/occupations/Management/SupplyChainManagers)
- [Logistics Analyst](/occupations/Business/LogisticsAnalysts)
- [Production Manager](/occupations/ProductionManagers)
- [Warehouse Manager](/occupations/WarehouseManagers)

## Related Departments

- Supply Chain & Logistics
- Manufacturing & Production
- Quality Assurance

## Industry Variations

### Manufacturing
Emphasis on lean production, JIT inventory, and continuous improvement methodologies such as Six Sigma and Kaizen.

### Retail
Focus on omnichannel fulfillment, last-mile delivery optimization, and seasonal demand management.

### Automotive
Integration of complex multi-tier supplier networks with assembly line synchronization and recall management.

## KPIs & Metrics

| KPI | Description | Unit |
|-----|-------------|------|
| Cycle Time | Average time to complete schedule production orders and create lots process | Hours/Days |
| Completion Rate | Percentage of production orders and create lots activities completed on schedule | % |
| Quality Score | Accuracy and quality rating of production orders and create lots outputs | 1-10 Scale |
| Cost Efficiency | Cost per unit of production orders and create lots processed | $/Unit |
| On-Time Delivery | Percentage of deliverables completed within target timeline | % |

## Related Concepts

- ProductionOrders
- CreateLots


---

*Source: APQC PCF 10308 (4.3.1.4) - APQC*
