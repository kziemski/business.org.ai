...
sequenceDiagram
    ...
    MRA->>+SD: Document current processes
    SD->>+MRA: Establish performance metrics
    MRA->>+SD: Create baseline measurements
    SD->>+MA: Present baseline report
    MA-->>-SD: Approve baseline targets
```
