...
sequenceDiagram
    ...
    BArc->>+SD: Gather model components
    SD->>+BArc: Structure model framework
    BArc->>+SD: Compile business model draft
    SD->>+CSO: Present assembled model
    CSO-->>-SD: Review model completeness
```
