---
id: AssessAndAnalyzeImpactOfEachOption
name: "Assess and analyze impact of each option"
code: "10048"
hierarchyId: "1.2.2.2"
type: Activity
level: 4
status: complete
parent: "1.2.2"
children: ["1.2.2.2.1", "1.2.2.2.2"]
---

# Assess and analyze impact of each option

> Scoping and probing to study the impact of strategic options for fulfilling the organization's objectives.

## Overview

Activity 1.2.2.2 is an activity within the Develop Vision and Strategy framework. 

Scoping and probing to study the impact of strategic options for fulfilling the organization's objectives. Estimate a measure of the impact effectuated by each set of strategic decisions, which comprise Define strategic options [10047]. Closely examine the consequences of each option.

## Process Hierarchy

```mermaid
graph TD
    C1["1.0 Develop Vision and Strategy"]
    C1 --> N1_2["1.2 Develop business strategy"]
    N1_2 --> N1_2_2["1.2.2 Define and evaluate strategic options..."]
    N1_2_2 --> N1_2_2_2["1.2.2.2 Assess and analyze impact of each option"]
    style N1_2_2_2 fill:#e1f5fe
    N1_2_2_2 --> N1_2_2_2_1["1.2.2.2.1 Identify implications for key op..."]
    N1_2_2_2 --> N1_2_2_2_2["1.2.2.2.2 Identify implications for key te..."]
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10048 |
| Hierarchy ID | 1.2.2.2 |
| Level | Activity |
| Parent | [1.2.2](../) |
| Sub-Processes | 2 |


## GraphDL Semantic Structure

```graphdl
assess.AndAnalyzeImpact.of.EachOption
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `assess` | Primary action |
| Object | `and analyze impact` | Direct object |
| Preposition | `of` | Relationship |
| PrepObject | `each option` | Indirect object |


## Process Flow

```mermaid
flowchart LR
    subgraph Inputs["Inputs"]
        I1["Upstream Process Outputs"]
        I2["Organizational Data"]
    end

    subgraph Process["Assess and analyze impact of each option"]
        S1["Identify implications for k..."]
        S2["Identify implications for k..."]
    end

    subgraph Outputs["Outputs"]
        O1["Process Deliverables"]
        O2["Updated Records"]
    end

    Inputs --> Process --> Outputs
    S1 --> S2
```

## Process Sequence

```mermaid
sequenceDiagram
    participant Analyst as Strategy Analyst
    participant Dir as Strategy Director
    participant Exec as Executive Team
    
    Analyst->>+Dir: Analyze strategic option impacts
    Dir->>+Analyst: Evaluate operational changes
    Analyst->>+Dir: Assess technology requirements
    Dir->>+Exec: Present impact analysis
    Exec-->>-Dir: Confirm implementation approach
```

## Sub-Processes

| Process | Hierarchy ID | Description |
|---------|-------------|-------------|
| [Identify implications for key operating model business elements that require change](./IdentifyImplicationsForKeyOperatingModelBusinessElementsThatRequireChange) | 1.2.2.2.1 | Determine impacts of elements such as staffing, skills, training, new markets, technology, or polici |
| [Identify implications for key technology aspects](./IdentifyImplicationsForKeyTechnologyAspects) | 1.2.2.2.2 | Determining key factors for technology ROI, benefits, architecture, etc |


## Related Concepts

- Impact
- Option
- Impact
- Option


---

*Source: APQC PCF 10048 (1.2.2.2) - APQC*
