---
id: CalculateDeferredTaxes
name: "Calculate deferred taxes"
code: "10933"
hierarchyId: "9.9.2.4"
type: Activity
level: 4
status: complete
parent: "9.9.2"
---

# Calculate deferred taxes

> Calculating the income that has been realized when the tax on that income has not.

## Overview

Activity 9.9.2.4 is an activity within the Tax Management domain of the Manage Financial Resources framework.

Calculating the income that has been realized when the tax on that income has not. This activity plays a critical role in ensuring that the organization maintains sound financial governance, operational efficiency, and regulatory compliance. It supports upstream planning and downstream execution by providing structured outputs that inform decision-making across finance and business operations. Effective execution of this activity requires coordination among finance professionals, process owners, and leadership stakeholders to ensure accuracy, timeliness, and alignment with organizational objectives.

## Process Hierarchy

```mermaid
graph TD
    C9["9.0 Manage Financial Resources"]
    C9 --> N9_9["9.9 Manage taxes"]
    N9_9 --> N9_9_2["9.9.2 Process taxes"]
    N9_9_2 --> N9_9_2_4["9.9.2.4 Calculate deferred taxes"]
    style N9_9_2_4 fill:#e1f5fe
```

## Process Flow

```mermaid
flowchart LR
    A["Identify requirements for deferred taxes"] --> B["Gather relevant data and documentation"]
    B --> C["Calculate deferred taxes"]
    C --> D["Review and validate results"]
    D --> E["Obtain necessary approvals"]
    E --> F["Document and archive outputs"]
    F --> G["Completed"]
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10933 |
| Hierarchy ID | 9.9.2.4 |
| Level | Activity |
| Parent | [9.9.2](../) |
| Sub-Processes | 0 |

## GraphDL Semantic Structure

```graphdl
calculate.DeferredTaxes
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `calculate` | Primary action |
| Object | `deferred taxes` | Direct object |

## RACI Matrix

| Activity | Responsible | Accountable | Consulted | Informed |
|----------|-------------|-------------|-----------|----------|
| Prepare tax returns | Tax Analyst | Tax Director | External Tax Advisor | CFO |
| Manage tax provision | Tax Manager | Tax Director | Controller | CFO |
| Plan tax strategy | Tax Director | CFO | Legal Counsel | Board |

## Related Occupations

- [Financial Managers](/occupations/Management/FinancialManagers)
- [Accountants and Auditors](/occupations/Business/Financial/AccountantsAndAuditors)
- [Tax Preparers](/occupations/Business/TaxPreparers)
- [Tax Examiners and Collectors](/occupations/TaxExaminersCollectorsAndRevenueAgents)
- [Financial Analysts](/occupations/Business/Financial/FinancialAnalysts)

## Related Departments

- Tax
- Legal & Compliance
- Finance & Accounting

## Industry Variations

### Multinational Corporations

Tax management spans transfer pricing, permanent establishment risks, BEPS compliance, and multi-jurisdiction filing obligations.

### Real Estate

Tax planning leverages depreciation strategies, 1031 exchanges, opportunity zones, and REIT structure optimization.

### Technology

Manages R&D tax credits, IP holding structures, and digital services tax obligations across jurisdictions.

## KPIs & Metrics

| Metric | Description | Target |
|--------|-------------|--------|
| Effective Tax Rate | Total tax expense as percentage of pre-tax income | At or below statutory rate |
| Tax Filing Timeliness | On-time filing of all tax returns | 100% |
| Tax Audit Adjustments | Value of audit-imposed adjustments | < 1% of tax liability |
| Tax Provision Accuracy | Accuracy of quarterly tax provision | > 98% |

## Process Sequence

```mermaid
sequenceDiagram
    autonumber
    Tax Analyst->>+ERP System: Extract book-tax differences
    ERP System-->>-Tax Analyst: Return difference data
    Tax Analyst->>+Tax Analyst: Calculate deferred taxes
    Note over Tax Analyst: Apply ASC 740 rules
    Tax Analyst->>+Tax Manager: Review DTA/DTL
    Tax Manager-->>-Tax Analyst: Approve calculations
    Tax Analyst->>+ERP System: Post tax provision
    ERP System-->>-Tax Analyst: Confirm posting
    Tax Analyst->>+Controller: Document workpapers
    Controller-->>-Tax Analyst: Confirm documentation
```

## Related Concepts

- DeferredTaxes

---

*Source: APQC PCF 10933 (9.9.2.4) - APQC*