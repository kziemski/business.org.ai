...
```mermaid
flowchart LR
    A["Identify requirements for and distribute manual checks"] --> B["Gather relevant data and documentation"]
    B --> C["Process and distribute manual checks"]
...
sequenceDiagram
    autonumber
    Payroll Clerk->>+HRIS: Calculate gross pay
    HRIS-->>-Payroll Clerk: Return pay calculations
    Payroll Clerk->>+HRIS: Calculate deductions
    HRIS-->>-Payroll Clerk: Return net pay
    Payroll Clerk->>+Payroll Manager: Submit payroll for approval
    Payroll Manager-->>-Payroll Clerk: Approve payroll run
    Payroll Clerk->>+HRIS: Execute payment file
    HRIS-->>-Payroll Clerk: Confirm transmission
    Payroll Clerk->>+Employee: Notify employees
    Employee-->>-Payroll Clerk: Acknowledge notification
    Payroll Clerk->>+Controller: Post payroll entries
    Controller-->>-Payroll Clerk: Confirm GL posting
```
