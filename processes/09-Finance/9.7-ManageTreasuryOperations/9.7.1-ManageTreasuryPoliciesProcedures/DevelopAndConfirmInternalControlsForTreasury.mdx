---
id: DevelopAndConfirmInternalControlsForTreasury
name: "Develop and confirm internal controls for treasury"
code: "10891"
hierarchyId: "9.7.1.7"
type: Activity
level: 4
status: complete
parent: "9.7.1"
---

# Develop and confirm internal controls for treasury

> Creating and managing the internal control systems for investments in bonds, currencies, and financial derivatives to verify procedures.

## Overview

Activity 9.7.1.7 is an activity within the Treasury Management domain of the Manage Financial Resources framework.

Creating and managing the internal control systems for investments in bonds, currencies, and financial derivatives to verify procedures. This activity plays a critical role in ensuring that the organization maintains sound financial governance, operational efficiency, and regulatory compliance. It supports upstream planning and downstream execution by providing structured outputs that inform decision-making across finance and business operations. Effective execution of this activity requires coordination among finance professionals, process owners, and leadership stakeholders to ensure accuracy, timeliness, and alignment with organizational objectives.

## Process Hierarchy

```mermaid
graph TD
    C9["9.0 Manage Financial Resources"]
    C9 --> N9_7["9.7 Manage treasury operations"]
    N9_7 --> N9_7_1["9.7.1 Manage treasury policies and procedures"]
    N9_7_1 --> N9_7_1_7["9.7.1.7 Develop and confirm internal controls..."]
    style N9_7_1_7 fill:#e1f5fe
```

## Process Flow

```mermaid
flowchart LR
    A["Identify requirements for and confirm internal controls for treasury"] --> B["Gather relevant data and documentation"]
    B --> C["Develop and confirm internal controls for treasury"]
    C --> D["Review and validate results"]
    D --> E["Obtain necessary approvals"]
    E --> F["Document and archive outputs"]
    F --> G["Completed"]
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10891 |
| Hierarchy ID | 9.7.1.7 |
| Level | Activity |
| Parent | [9.7.1](../) |
| Sub-Processes | 0 |

## GraphDL Semantic Structure

```graphdl
develop.AndConfirmInternalControls.for.Treasury
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `develop` | Primary action |
| Object | `and confirm internal controls` | Direct object |
| Preposition | `for` | Relationship |
| PrepObject | `treasury` | Indirect object |

## RACI Matrix

| Activity | Responsible | Accountable | Consulted | Informed |
|----------|-------------|-------------|-----------|----------|
| Manage cash positions | Treasury Analyst | Treasurer | Controller | CFO |
| Execute investments | Treasury Analyst | Treasurer | Investment Committee | CFO |
| Manage bank relationships | Treasurer | CFO | Legal | Controller |

## Related Occupations

- [Financial Managers](/occupations/Management/FinancialManagers)
- [Accountants and Auditors](/occupations/Business/Financial/AccountantsAndAuditors)
- [Treasurers and Controllers](/occupations/Treasurers)
- [Financial Analysts](/occupations/Business/Financial/FinancialAnalysts)
- [Loan Officers](/occupations/Business/LoanOfficers)

## Related Departments

- Treasury
- Corporate Finance
- Finance & Accounting

## Industry Variations

### Banking

Treasury operations include interbank lending, reserve management, and liquidity coverage ratio compliance.

### Multinational Corporations

Manages multi-currency cash pools, cross-border netting, and foreign exchange hedging strategies.

### Energy

Treasury handles commodity hedging, project finance structures, and long-term power purchase agreement management.

## KPIs & Metrics

| Metric | Description | Target |
|--------|-------------|--------|
| Cash Forecast Accuracy | Accuracy of short-term cash forecasts | > 95% |
| Idle Cash Ratio | Percentage of cash not optimally deployed | < 5% |
| Borrowing Cost | Weighted average cost of debt | Below benchmark |
| FX Hedge Effectiveness | Effectiveness of currency hedging programs | > 80% |

## Process Sequence

```mermaid
sequenceDiagram
    autonumber
    Treasury Analyst->>+ERP System: Initiate and confirm internal controls for treasury
    ERP System-->>-Treasury Analyst: Return treasury data
    Treasury Analyst->>+Treasurer: Submit for approval
    Treasurer-->>-Treasury Analyst: Approve transaction
    Treasury Analyst->>+Bank: Execute with counterparty
    Bank-->>-Treasury Analyst: Confirm execution
    Treasury Analyst->>+Controller: Update accounting records
    Controller-->>-Treasury Analyst: Confirm posting
```

## Related Concepts

- InternalControls
- Treasury
- InternalControls
- Treasury

---

*Source: APQC PCF 10891 (9.7.1.7) - APQC*