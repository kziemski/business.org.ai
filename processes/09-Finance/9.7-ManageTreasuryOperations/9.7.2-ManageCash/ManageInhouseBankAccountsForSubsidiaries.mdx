---
id: ManageInHouseBankAccountsForSubsidiaries
name: "Manage in-house bank accounts for subsidiaries"
code: "10901"
hierarchyId: "9.7.2.10"
type: Activity
level: 4
status: complete
parent: "9.7.2"
---

# Manage in-house bank accounts for subsidiaries

> Maintaining subsidiaries' company accounts opened with bank inside the corporation.

## Overview

Activity 9.7.2.10 is an activity within the Treasury Management domain of the Manage Financial Resources framework.

Maintaining subsidiaries' company accounts opened with bank inside the corporation. This activity plays a critical role in ensuring that the organization maintains sound financial governance, operational efficiency, and regulatory compliance. It supports upstream planning and downstream execution by providing structured outputs that inform decision-making across finance and business operations. Effective execution of this activity requires coordination among finance professionals, process owners, and leadership stakeholders to ensure accuracy, timeliness, and alignment with organizational objectives.

## Process Hierarchy

```mermaid
graph TD
    C9["9.0 Manage Financial Resources"]
    C9 --> N9_7["9.7 Manage treasury operations"]
    N9_7 --> N9_7_2["9.7.2 Manage cash"]
    N9_7_2 --> N9_7_2_10["9.7.2.10 Manage in-house bank accounts for sub..."]
    style N9_7_2_10 fill:#e1f5fe
```

## Process Flow

```mermaid
flowchart LR
    A["Identify requirements for in-house bank accounts for subsidiaries"] --> B["Gather relevant data and documentation"]
    B --> C["Manage in-house bank accounts for subsidiaries"]
    C --> D["Review and validate results"]
    D --> E["Obtain necessary approvals"]
    E --> F["Document and archive outputs"]
    F --> G["Completed"]
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10901 |
| Hierarchy ID | 9.7.2.10 |
| Level | Activity |
| Parent | [9.7.2](../) |
| Sub-Processes | 0 |

## GraphDL Semantic Structure

```graphdl
manage.InhouseBankAccounts.for.Subsidiaries
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `manage` | Primary action |
| Object | `in-house bank accounts` | Direct object |
| Preposition | `for` | Relationship |
| PrepObject | `subsidiaries` | Indirect object |

## RACI Matrix

| Activity | Responsible | Accountable | Consulted | Informed |
|----------|-------------|-------------|-----------|----------|
| Manage cash positions | Treasury Analyst | Treasurer | Controller | CFO |
| Execute investments | Treasury Analyst | Treasurer | Investment Committee | CFO |
| Manage bank relationships | Treasurer | CFO | Legal | Controller |

## Related Occupations

- [Financial Managers](/occupations/Management/FinancialManagers)
- [Accountants and Auditors](/occupations/Business/Financial/AccountantsAndAuditors)
- [Treasurers and Controllers](/occupations/Treasurers)
- [Financial Analysts](/occupations/Business/Financial/FinancialAnalysts)
- [Loan Officers](/occupations/Business/LoanOfficers)

## Related Departments

- Treasury
- Corporate Finance
- Finance & Accounting

## Industry Variations

### Banking

Treasury operations include interbank lending, reserve management, and liquidity coverage ratio compliance.

### Multinational Corporations

Manages multi-currency cash pools, cross-border netting, and foreign exchange hedging strategies.

### Energy

Treasury handles commodity hedging, project finance structures, and long-term power purchase agreement management.

## KPIs & Metrics

| Metric | Description | Target |
|--------|-------------|--------|
| Cash Forecast Accuracy | Accuracy of short-term cash forecasts | > 95% |
| Idle Cash Ratio | Percentage of cash not optimally deployed | < 5% |
| Borrowing Cost | Weighted average cost of debt | Below benchmark |
| FX Hedge Effectiveness | Effectiveness of currency hedging programs | > 80% |

## Process Sequence

```mermaid
sequenceDiagram
    autonumber
    Treasury Analyst->>+Bank: Request account information
    Bank-->>-Treasury Analyst: Return account details
    Treasury Analyst->>+ERP System: Update bank master data
    ERP System-->>-Treasury Analyst: Confirm update
    Treasury Analyst->>+Treasurer: Review account setup
    Treasurer-->>-Treasury Analyst: Approve configuration
    Treasury Analyst->>+Bank: Confirm account status
    Bank-->>-Treasury Analyst: Acknowledge confirmation
    Treasury Analyst->>+Controller: Report bank relationships
    Controller-->>-Treasury Analyst: Confirm reporting
```

## Related Concepts



---

*Source: APQC PCF 10901 (9.7.2.10) - APQC*