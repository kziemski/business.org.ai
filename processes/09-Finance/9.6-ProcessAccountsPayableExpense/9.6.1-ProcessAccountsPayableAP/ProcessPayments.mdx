...
sequenceDiagram
    autonumber
    AP Clerk->>+ERP System: Retrieve approved invoices
    ERP System-->>-AP Clerk: Return payment batch
    AP Clerk->>+AP Manager: Submit payment run for approval
    AP Manager-->>-AP Clerk: Approve payment batch
    AP Clerk->>+Treasury: Request fund availability
    Treasury-->>-AP Clerk: Confirm funds available
    AP Clerk->>+ERP System: Execute payment transactions
    ERP System-->>-AP Clerk: Generate payment confirmations
    ERP System->>+Vendor: Transmit payment
    Vendor-->>-ERP System: Acknowledge receipt
    AP Clerk->>+Controller: Report payment activity
