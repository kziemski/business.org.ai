...
```mermaid
flowchart LR
    A["Identify requirements for payments"] --> B["Gather relevant data and documentation"]
    B --> C["Approve payments"]
...
sequenceDiagram
    autonumber
    AP Clerk->>+ERP System: Submit invoices for approval
    ERP System-->>-AP Clerk: Return pending approvals
    AP Clerk->>+AP Manager: Route for approval
    AP Manager->>+ERP System: Review invoice details
    ERP System-->>-AP Manager: Display supporting documents
    AP Manager-->>-AP Clerk: Return approval decision
    alt Approved
        AP Clerk->>+ERP System: Update approval status
        ERP System-->>-AP Clerk: Confirm status update
    else Rejected
        AP Clerk->>+Vendor: Return invoice for correction
        Vendor-->>-AP Clerk: Acknowledge rejection
    end
    AP Clerk->>+Controller: Update approval audit trail
    Controller-->>-AP Clerk: Confirm audit record
```
