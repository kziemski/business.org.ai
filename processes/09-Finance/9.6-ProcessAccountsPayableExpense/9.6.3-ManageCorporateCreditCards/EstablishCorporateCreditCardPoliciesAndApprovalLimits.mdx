...
```mermaid
flowchart LR
    A["Identify requirements for corporate credit card policies and approval limits"] --> B["Gather relevant data and documentation"]
    B --> C["Establish corporate credit card policies and approval limits"]
...
sequenceDiagram
    autonumber
    AR Clerk->>+Customer: Initiate corporate credit card policies and approval limits
    Customer-->>-AR Clerk: Return relevant data
    AR Clerk->>+AR Manager: Submit for review
    AR Manager-->>-AR Clerk: Approve processing
    AR Clerk->>+Customer: Execute establish action
