...
sequenceDiagram
    autonumber
    Control Analyst->>+Process Owner: Compile control results
    Process Owner-->>-Control Analyst: Return assessment data
    Control Analyst->>+Controller: Review findings
    Controller-->>-Control Analyst: Approve report
    Control Analyst->>+Auditor: Submit to auditors
    Auditor-->>-Control Analyst: Acknowledge receipt
    Control Analyst->>+CFO: Present to management
    CFO-->>-Control Analyst: Request action items
    Control Analyst->>+Board: Brief the board
    Board-->>-Control Analyst: Confirm briefing
```
