...
```mermaid
flowchart LR
    A["Identify requirements for to regulators, share-/debt-holders, securities exchanges, etc."] --> B["Gather relevant data and documentation"]
    B --> C["Report to regulators, share-/debt-holders, securities exchanges, etc."]
...
sequenceDiagram
    autonumber
    Treasury Analyst->>+ERP System: Compile control results
    ERP System-->>-Treasury Analyst: Return assessment data
    Treasury Analyst->>+Treasurer: Review findings
    Treasurer-->>-Treasury Analyst: Approve report
    Treasury Analyst->>+Bank: Submit to auditors
    Bank-->>-Treasury Analyst: Acknowledge receipt
    Treasury Analyst->>+Controller: Present to management
    Controller-->>-Treasury Analyst: Request action items
    Treasury Analyst->>+CFO: Brief the board
    CFO-->>-Treasury Analyst: Confirm briefing
```
