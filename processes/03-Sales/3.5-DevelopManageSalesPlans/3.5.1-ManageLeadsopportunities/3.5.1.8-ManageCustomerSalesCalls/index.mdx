---
id: ManageCustomerSalesCalls
name: "Manage customer sales calls"
code: "10184"
hierarchyId: "3.5.1.8"
type: Activity
level: 4
status: complete
parent: "3.5.1"
children: ["3.5.1.8.1", "3.5.1.8.2", "3.5.1.8.3", "3.5.1.8.4", "3.5.1.8.5"]
---

# Manage customer sales calls

> Managing the entire sales process, from using leads to open sales to closing sales and creating records.

## Overview

Activity 3.5.1.8 is an activity within the Market and Sell Products and Services framework. 

Managing the entire sales process, from using leads to open sales to closing sales and creating records. Govern all sales activities. Make sales calls based on leads and preparatory work (drafting terms of the sale, creating proposals, suggesting prices, etc.). Close the sale, along with any administrative activities related to data entry and the processing of the sale.

## Process Hierarchy

```mermaid
graph TD
    C3["3.0 Market and Sell Products and Services"]
    C3 --> N3_5["3.5 Develop and manage sales plans"]
    N3_5 --> N3_5_1["3.5.1 Manage leads/opportunities"]
    N3_5_1 --> N3_5_1_8["3.5.1.8 Manage customer sales calls"]
    style N3_5_1_8 fill:#e1f5fe
    N3_5_1_8 --> N3_5_1_8_1["3.5.1.8.1 Perform sales calls"]
    N3_5_1_8 --> N3_5_1_8_2["3.5.1.8.2 Perform pre-sales activities"]
    N3_5_1_8 --> N3_5_1_8_3["3.5.1.8.3 Manage customer meetings/workshops"]
    N3_5_1_8 --> N3_5_1_8_4["3.5.1.8.4 Close the sale"]
    N3_5_1_8 --> N3_5_1_8_5["3.5.1.8.5 Record outcome of sales process"]
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10184 |
| Hierarchy ID | 3.5.1.8 |
| Level | Activity |
| Parent | [3.5.1](../) |
| Sub-Processes | 5 |


## GraphDL Semantic Structure

```graphdl
manage.CustomerSalesCalls
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `manage` | Primary action |
| Object | `customer sales calls` | Direct object |


## Process Flow

```mermaid
flowchart LR
    subgraph Inputs["Inputs"]
        I1["Upstream Process Outputs"]
        I2["Organizational Data"]
    end

    subgraph Process["Manage customer sales calls"]
        S1["Perform sales calls"]
        S2["Perform pre-sales activities"]
        S3["Manage customer meetings/wo..."]
        S4["Close the sale"]
        S5["Record outcome of sales pro..."]
    end

    subgraph Outputs["Outputs"]
        O1["Process Deliverables"]
        O2["Updated Records"]
    end

    Inputs --> Process --> Outputs
    S1 --> S2
    S2 --> S3
    S3 --> S4
    S4 --> S5
```

## Process Sequence

```mermaid
sequenceDiagram
    participant SR as Sales Rep
    participant AE as Account Executive
    participant C as Customer
    participant SM as Sales Manager
    participant CRM as CRM System

    SR->>CRM: Review qualified leads
    SR->>C: Initiate sales call
    C-->>SR: Express interest and needs
    SR->>AE: Escalate high-value opportunity
    AE->>C: Conduct pre-sales discovery
    AE->>C: Schedule customer meeting
    C-->>AE: Attend product demonstration
    AE->>C: Present proposal and pricing
    C-->>AE: Negotiate terms
    AE->>SM: Request deal approval
    SM-->>AE: Approve final terms
    AE->>C: Close the sale
    C-->>AE: Sign agreement
    AE->>CRM: Record sales outcome
```

## Sub-Processes

| Process | Hierarchy ID | Description |
|---------|-------------|-------------|
| [Perform sales calls](./PerformSalesCalls) | 3.5.1.8.1 | Communicating with customers and prospects with the intent of creating sales opportunities |
| [Perform pre-sales activities](./PerformPresalesActivities) | 3.5.1.8.2 | Capitalizing on sales calls by pitching on bids and closing deals |
| [Manage customer meetings/workshops](./ManageCustomerMeetingsworkshops) | 3.5.1.8.3 | Arranging and leading meetings, seminars, workshops and training events with customers to educate th |
| [Close the sale](./CloseTheSale) | 3.5.1.8.4 | Formalizing a sale by reaching an agreement on terms of the deal |
| [Record outcome of sales process](./RecordOutcomeOfSalesProcess) | 3.5.1.8.5 | Completing all the paper-work associated with the sale of its products/services |


## Related Concepts

- CustomerSalesCalls


---

*Source: APQC PCF 10184 (3.5.1.8) - APQC*
