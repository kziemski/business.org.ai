---
id: SolicitFeedbackFromCustomerOnServiceDeliverySatisfaction
name: "Solicit feedback from customer on service delivery satisfaction"
code: "20031"
hierarchyId: "5.1.1.4"
type: Activity
level: 4
status: complete
parent: "5.1.1"
---

# Solicit feedback from customer on service delivery satisfaction

> Engaging the customer post delivery to gauge the effectiveness of services rendered in order to improve on key delivery functions going forward.

## Overview

This activity encompasses the end-to-end process of solicit feedback from customer on service delivery satisfaction within the service delivery and governance domain. It involves coordinating cross-functional teams, applying standardized methodologies, and leveraging organizational data to ensure consistent and effective outcomes. The process is aligned with the broader Deliver Services framework (APQC 5.1.1.4) and supports strategic objectives by translating operational requirements into actionable procedures.

Effective execution of this activity requires clear ownership, well-defined inputs and outputs, and continuous monitoring against established benchmarks. Organizations that excel at this process typically integrate it with upstream planning activities and downstream performance measurement, creating a feedback loop that drives ongoing improvement and adaptation to changing business conditions.


## Process Hierarchy

```mermaid
graph TD
    C5["5.0 Deliver Services"]
    C5 --> N5_1["5.1 Establish service delivery governance..."]
    N5_1 --> N5_1_1["5.1.1 Establish service delivery governance"]
    N5_1_1 --> N5_1_1_4["5.1.1.4 Solicit feedback from customer on ser..."]
    style N5_1_1_4 fill:#e1f5fe
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 20031 |
| Hierarchy ID | 5.1.1.4 |
| Level | Activity |
| Parent | [5.1.1](../) |
| Sub-Processes | 0 |


## GraphDL Semantic Structure

```graphdl
solicit.Feedback.from.CustomerOnServiceDeliverySatisfaction
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `solicit` | Primary action |
| Object | `feedback` | Direct object |
| Preposition | `from` | Relationship |
| PrepObject | `customer on service delivery satisfaction` | Indirect object |



## Process Flow

```mermaid
flowchart LR
    subgraph Inputs["Inputs"]
        I1["Requirements & Standards"]
        I2["Organizational Data"]
        I3["Stakeholder Input"]
    end

    subgraph Process["Solicit feedback from customer on service delivery satisfaction"]
        P1["Gather Requirements"]
        P2["Analyze Current State"]
        P3["Solicit feedback from customer on service delivery satisfaction"]
        P4["Validate & Review"]
        P5["Document & Communicate"]
    end

    subgraph Outputs["Outputs"]
        O1["Updated Deliverables"]
        O2["Performance Records"]
        O3["Stakeholder Communication"]
    end

    Inputs --> Process --> Outputs
    P1 --> P2 --> P3 --> P4 --> P5
```


## Process Sequence

```mermaid
sequenceDiagram
    participant ServiceRep as Service Rep
    participant Technician as Technician
    participant Customer as Customer
    participant Dispatcher as Dispatcher

    ServiceRep->>Technician: Initiate solicit feedback from customer on service delivery satisfaction request
    Technician->>Technician: Analyze requirements
    Technician->>Customer: Request validation
    Customer->>Technician: Provide feedback
    Technician->>Dispatcher: Update status
    Dispatcher->>ServiceRep: Confirm completion
```
## RACI Matrix

| Activity | Service Delivery Manager | Operations Director | Quality Assurance Team | Business Development |
|----------|:-:|:-:|:-:|:-:|
| Gather Requirements | R | A | C | I |
| Analyze Current State | R | I | C | I |
| Solicit feedback from customer on service delivery satisfaction | R | A | C | I |
| Validate & Review | C | A | R | I |
| Document & Communicate | R | I | I | C |

## Related Occupations

- [Service Delivery Manager](/occupations/ServiceDeliveryManagers)
- [Operations Manager](/occupations/Management/OperationsManagers)
- [Business Analyst](/occupations/BusinessAnalysts)
- [Quality Assurance Specialist](/occupations/QualityAssuranceSpecialists)

## Related Departments

- Service Operations
- Business Development
- Quality Management

## Industry Variations

### Professional Services
Focus on billable utilization, client engagement models, and knowledge management across consulting and advisory practices.

### Healthcare
Emphasis on patient outcomes, regulatory compliance (HIPAA), and care coordination across multidisciplinary teams.

### Financial Services
Prioritization of risk management, regulatory adherence, and digital transformation of client-facing services.

## KPIs & Metrics

| KPI | Description | Unit |
|-----|-------------|------|
| Cycle Time | Average time to complete solicit feedback process | Hours/Days |
| Completion Rate | Percentage of feedback activities completed on schedule | % |
| Quality Score | Accuracy and quality rating of feedback outputs | 1-10 Scale |
| Cost Efficiency | Cost per unit of feedback processed | $/Unit |
| Service Level Agreement (SLA) Compliance | Percentage of activities meeting SLA targets | % |

## Related Concepts

- Feedback
- CustomerOnServiceDeliverySatisfaction


---

*Source: APQC PCF 20031 (5.1.1.4) - APQC*
