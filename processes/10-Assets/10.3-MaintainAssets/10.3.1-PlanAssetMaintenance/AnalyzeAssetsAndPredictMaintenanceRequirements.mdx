---
id: AnalyzeAssetsAndPredictMaintenanceRequirements
name: "Analyze assets and predict maintenance requirements"
code: "10967"
hierarchyId: "10.3.1.2"
type: Activity
level: 4
status: complete
parent: "10.3.1"
---

# Analyze assets and predict maintenance requirements

> Evaluating assets in order to project future requirements for maintenance.

## Overview

The analyze assets and predict maintenance requirements process is a critical component of the Asset Management function within an organization. It encompasses the systematic approach to analyze assets and predict maintenance requirements, ensuring that all activities are performed consistently, efficiently, and in alignment with organizational objectives. This process establishes the framework through which analyze assets and predict maintenance requirements is executed, monitored, and continuously improved to deliver value across the enterprise.

Within the APQC Process Classification Framework (hierarchy 10.3.1.2), this activity supports the broader "Acquire, Construct, and Manage Assets" category. Effective execution requires cross-functional collaboration, clear accountability, and robust governance mechanisms. Organizations that mature this process typically see improved operational performance, reduced risk exposure, and stronger alignment between tactical activities and strategic goals.


## Process Hierarchy

```mermaid
graph TD
    C10["10.0 Acquire, Construct, and Manage Assets"]
    C10 --> N10_3["10.3 Maintain assets"]
    N10_3 --> N10_3_1["10.3.1 Plan asset maintenance"]
    N10_3_1 --> N10_3_1_2["10.3.1.2 Analyze assets and predict maintenanc..."]
    style N10_3_1_2 fill:#e1f5fe
```

## Key Statistics

| Metric | Value |
|--------|-------|
| APQC Code | 10967 |
| Hierarchy ID | 10.3.1.2 |
| Level | Activity |
| Parent | [10.3.1](../) |
| Sub-Processes | 0 |


## GraphDL Semantic Structure

```graphdl
analyze.AssetsAndPredictMaintenanceRequirements
```

| Component | Value | Description |
|-----------|-------|-------------|
| Verb | `analyze` | Primary action |
| Object | `assets and predict maintenance requirements` | Direct object |


## Process Flow

```mermaid
flowchart LR
    A["Identify Requirements"] --> B["Plan assets and predict maintenance requirements"]
    B --> C["Execute Analyze Activities"]
    C --> D["Review & Validate"]
    D --> E["Document & Report"]
    D -- "Issues Found" --> C
```


## Process Sequence

```mermaid
sequenceDiagram
    participant FacilitiesManager as Facilities Manager
    participant MaintenanceTech as Maintenance Tech
    participant Vendor as Vendor

    FacilitiesManager->>MaintenanceTech: Initiate analyze assets and predict maintenance requirements request
    MaintenanceTech->>MaintenanceTech: Process request
    MaintenanceTech->>Vendor: Request validation
    Vendor->>FacilitiesManager: Confirm completion
```
## RACI Matrix

| Activity | Manager | Manager | Manager | Supervisor |
|----------|------|------|------|------|
| Planning & Scoping | R | A | C | I |
| Execution | A | C | I | R |
| Review & Approval | C | I | R | A |
| Reporting | I | R | A | C |

## Related Occupations

- [Facilities Manager](/occupations/FacilitiesManager)
- [Asset Manager](/occupations/AssetManager)
- [Property Manager](/occupations/PropertyManager)
- [Maintenance Supervisor](/occupations/MaintenanceSupervisor)

## Related Departments

- Facilities Management
- Capital Planning
- Operations

## Industry Variations

### Manufacturing

Heavy emphasis on equipment lifecycle management, predictive maintenance scheduling, and capital equipment depreciation tracking.

### Real Estate

Focus on property portfolio optimization, lease management, tenant improvements, and market valuation assessments.

### Healthcare

Strict regulatory compliance for medical equipment, biomedical asset tracking, and facility accreditation requirements.

## KPIs & Metrics

| KPI | Target | Measurement Frequency |
|-----|--------|----------------------|
| Asset Utilization Rate | > 85% | Monthly |
| Maintenance Cost as % of Asset Value | < 3% | Quarterly |
| Mean Time Between Failures (MTBF) | Increasing trend | Monthly |
| Capital Project On-Time Delivery | > 90% | Per Project |

## Related Concepts

- Assets
- PredictMaintenanceRequirements


---

*Source: APQC PCF 10967 (10.3.1.2) - APQC*
