...
sequenceDiagram
    ...
    HRS->>HRIS: Identify redeployment candidates
    HRIS-->>HRS: Return eligible employees
    HRS->>HRM: Review redeployment options
    HRM->>HM: Coordinate position availability
    HM-->>HRM: Confirm open positions
    HRM->>Emp: Present redeployment opportunity
    Emp-->>HRM: Express interest
    HRS->>HRIS: Process transfer request
    HRIS-->>HRS: Update employee record
    HRS->>HM: Confirm redeployment
    HM->>Emp: Welcome to new role
```
