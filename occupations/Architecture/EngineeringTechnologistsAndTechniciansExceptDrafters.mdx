---
id: EngineeringTechnologistsAndTechniciansExceptDrafters
name: "Engineering Technologists and Technicians, Except Drafters, All Other"
code: "17-3029.00"
type: Occupation
status: enriched
---

# Engineering Technologists and Technicians, Except Drafters, All Other

> All engineering technologists and technicians, except drafters, not listed separately.

## Overview

Engineering Technologists and Technicians, Except Drafters professionals serve a vital function within the Architecture and Engineering field. They bring specialized skills and knowledge to their roles, contributing to organizational objectives and societal needs.

These practitioners work in varied environments, adapting their expertise to meet specific requirements of their industry and employer. The role requires ongoing professional development to maintain competency and respond to changing demands.

Career paths in this field offer opportunities for advancement through experience, additional education, and specialized certifications. Employment prospects are influenced by industry trends, technological change, and workforce demographics.

## Classification Hierarchy

```mermaid
graph TD
    Cat17["17 - Architecture and Engineering"] --> Group173029["17-3029 Engineering Technologists and Technicians, Except Drafters, All Other"]
    Group173029 --> Current["17-3029.00 Engineering Technologists and Technicians, Except Drafters, All Other"]
    style Current fill:#e1f5fe
```

## Key Statistics

| Metric | Value |
|--------|-------|
| SOC Code | 17-3029.00 |
| Job Zone | N/A |
| Category | [Architecture and Engineering](/occupations/Architecture/index) |
| Core Tasks | N/A+ |
| Salary Range | $55,000 - $140,000 |
| Median Salary | $85,000 |
| Growth Outlook | 4% (As fast as average) |
| Source | O*NET |

## Core Tasks

```mermaid
mindmap
    root(("EngineeringTechnologistsAndTechniciansExceptDrafters"))
        Examine
            Structures
            Vehicles
            Aircraft
            Trains
            Nuclear Reactors
            Bridges
            Dams
            Pipelines
        Evaluate
            Test Results
            Material Properties
            Using Radio Astronomy
            Voltage
            Amperage Measurement
            Rheometric Flow Measurement
        Develop
            New Non Destructive Testing Methods
            Acoustic Emission Testing
            Leak Testing
            Thermal
            Infrared Testing
        Use
            New Non Destructive Testing Methods
            Acoustic Emission Testing
            Leak Testing
            Thermal
            Infrared Testing
        Visually
            Examine Materials
            Structures
            Components
        Interpret
            Results
            Test Results
        Document
            Non Destructive Testing Methods
            Processes
        Identify
            Defects
        Make
            Radiographic Images
        Prepare
            Reports
```

### examine.Structures

Engineering Technologists and Technicians, Except Drafters, All Other examine structures as part of their core responsibilities.

**Actions:**
- `examine.Structures`
- `examine.Vehicles`
- `examine.Aircraft`
- `examine.Trains`

### evaluate.TestResults

Engineering Technologists and Technicians, Except Drafters, All Other evaluate test results as part of their core responsibilities.

**Actions:**
- `evaluate.TestResults.in.Accordance`
- `evaluate.TestResults.in.Standards`
- `evaluate.TestResults.in.Specifications`
- `evaluate.TestResults.in.Procedures`

### develop.NewNonDestructiveTestingMethods

Engineering Technologists and Technicians, Except Drafters, All Other develop new non destructive testing methods as part of their core responsibilities.

**Actions:**
- `develop.NewNonDestructiveTestingMethods`
- `develop.AcousticEmissionTesting`
- `develop.LeakTesting`
- `develop.Thermal`


### Technical Skills
- **Engineering Design** - Advanced
- **CAD/CAM** - Advanced
- **Technical Analysis** - Advanced

### Soft Skills
- **Communication** - Essential
- **Problem Solving** - Essential
- **Critical Thinking** - Important
- **Teamwork** - Important
- **Adaptability** - Important


## Skills & Competencies

### Technical Skills
- **Technical Design** - Expert
- **Engineering Analysis** - Advanced
- **CAD/BIM Software** - Advanced
- **Project Management** - Advanced
- **Code Compliance** - Advanced
- **Quality Assurance** - Proficient

### Soft Skills
- **Analytical Thinking** - Critical
- **Problem Solving** - Critical
- **Attention to Detail** - Essential
- **Teamwork** - Essential
- **Communication** - Essential

## Education & Certifications

| Requirement | Details |
|-------------|---------|
| Typical Education | Bachelor's degree in engineering, architecture, or related field |
| Work Experience | 2-4 years professional experience |
| On-the-Job Training | Moderate - technical specialization required |
| Certifications | Professional Engineer (PE), Architect License, or field-specific certifications |

## Career Progression

```mermaid
graph BT
    Entry["Junior Engineering Technologists and Technicians / Engineering Technician"] --> Mid["Engineering Technologists and Technicians / Project Engineer"]
    Mid --> Senior["Senior Engineering Technologists and Technicians / Lead Engineer"]
    Senior --> Lead["Principal Engineer / Engineering Manager / Director"]
```

## Industry Variations

### Private Sector Engineering
Design and development work for commercial clients. Engineering Technologists and Technicians professionals focus on product development, system design, and project delivery.

### Government and Infrastructure
Public works and infrastructure projects with emphasis on regulatory compliance and long-term sustainability.

### Construction and Field Engineering
On-site implementation and oversight of engineering designs. Strong focus on quality control and safety compliance.

### Consulting
Advisory services for diverse clients. Requires strong project management skills and ability to work across multiple simultaneous projects.

## Technology & Tools

- **Computer-Aided Design (CAD) software**
- **Building Information Modeling (BIM)**
- **Geographic Information Systems (GIS)**
- **Structural analysis software**
- **Project management tools**

## Related Occupations

```mermaid
graph LR
    Current["EngineeringTechnologistsAndTechniciansExceptDrafters"]

    subgraph SameCategory["Architecture and Engineering"]
        ArchitectsExceptLand["Architects, Except Landscape and..."]
        LandscapeArchitects["Landscape Architects"]
        CartographersAndPhot["Cartographers and Photogrammetrists"]
        Surveyors["Surveyors"]
    end

    Current --- SameCategory
```

## Industries

- [Engineering Services](/industries/Engineering) - High Employment
- [Construction](/industries/Construction) - High Employment
- [Manufacturing](/industries/Manufacturing) - Moderate Employment
- [Government](/industries/Government) - Moderate Employment

## Departments

This occupation typically works in:
- [Engineering](/departments/Engineering/index)
- [Design](/departments/Design)
- [Project Management](/departments/ProjectManagement)

## GraphDL Semantic Structure

```
Engineering Technologists and Technicians, Except Drafters, All Other perform:
- design.Systems.for.EngineeringTechnologistsandTechniciansApplications
- analyze.Requirements.for.ProjectSpecifications
- prepare.TechnicalDocumentation.for.Projects
- review.Designs.for.CodeCompliance
- coordinate.WithTeam.on.EngineeringProjects
```

---

*Source: O*NET 17-3029.00 - ONETOccupation*
